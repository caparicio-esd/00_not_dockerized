...
const { ORION_PORT, NODE_SERVER_PORT } = require("./constants");
...
const data = {
  ...
  notification: {
    http: {
      url: `http://host.docker.internal:${NODE_SERVER_PORT}/subscriptions`,
    },
  },
};
...
class Suscription {
  ...
  static async setSuscription() {
    await axios
      .post(`http://localhost:${ORION_PORT}/v2/subscriptions/`, { ...data }, { ...headers })
      .then((res) => {
        console.log();
      })
  ...
  static async checkSuscription() {
    await axios
      .get(`http://localhost:${ORION_PORT}/v2/subscriptions/`)
      .then(({ data }) => {
